...
space. k is the size of the k-cliques p is the number of threads Will print the
number of k-cliques.
*/
...
typedef struct {
  ...
  unsigned *rank; // ranking of the nodes according to degeneracy ordering
                  // unsigned *map;//oldID newID correspondance NOT USED IN THIS
                  // VERSION
} edgelist;
...
edgelist *readedgelist(char *input) {
  ...
  while (fscanf(file, "%u %u", &(el->edges[el->e].s), &(el->edges[el->e].t)) ==
         2) { // Add one edge
...
#pragma omp threadprivate(new, old)
  ...
  for (i = 0; i < sg->n[k - 1];
       i++) { // reodering adjacency list and computing new degrees
    v = old[i];
...
void kclique_thread(unsigned char l, subgraph *sg, unsigned long long *n) {
  ...
  if (l == 2) {
    for (i = 0; i < sg->n[2]; i++) { // list all edges
      ...
      for (j = u * sg->core; j < end; j++) {
        (*n)++; // listing here!!!  // NOTE THAT WE COULD DO (*n)+=g->d[2][u] to
                // be much faster (for counting only); !!!!!!!!!!!!!!!!!!
      }
    }
    return;
  }
  ...
  for (i = 0; i < sg->n[l]; i++) {
    ...
    for (j = 0; j < sg->n[l - 1];
         j++) { // reodering adjacency list and computing new degrees
      v = sg->nodes[l - 1][j];
