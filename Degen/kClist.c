...
This is an improvement of the 1985 algorithm of Chiba And Nishizeki detailed in
"Arboricity and subgraph listing".
...
"edgelist.txt" should contain the graph: one edge on each line separated by a
space. Will print the number of k-cliques.
*/
...
#define NLINKS                                                                 \
  100000000 // maximum number of edges for memory allocation, will increase if
            // needed
...
specialsparse *readedgelist(char *edgelist) {
  ...
  while (fscanf(file, "%u %u", &(g->edges[g->e].s), &(g->edges[g->e].t)) ==
         2) { // Add one edge
...
void ord_read(specialsparse *g, char *ordfile) {
  printf("Reading provided order\n");
  g->rank = malloc(g->n * sizeof(unsigned));
  FILE *file = fopen(ordfile, "r");
  unsigned r, u;
  for (u = 0; u < g->n; u++) {
    if (!fscanf(file, "%u", &r)) {
      printf("File is wrong after u=%u", u);
      break;
    }
    g->rank[u] = r;
  }
...
void ord_core(specialsparse *g) {
  printf("Computing core ordering\n");
...
void kclique(unsigned l, specialsparse *g, unsigned long long *n) {
  ...
  if (l == 2) {
    for (i = 0; i < g->ns[2]; i++) {      // for all remaining edges
      u = g->sub[2][i];
      // (*n)+=g->d[2][u];                // counting only (much faster)
      for (j = g->cd[u]; j < g->cd[u] + g->d[2][u]; j++)
        (*n)++;                           // listing
    }
    return;
  }

  for (i = 0; i < g->ns[l]; i++) {        // for each node still in the graph
    u = g->sub[l][i];
    g->ns[l - 1] = 0;                     // create an empty subgraph

    // check who belongs to the subgraph of u
    end = g->cd[u] + g->d[l][u];
    for (j = g->cd[u]; j < end; j++) {    // for each remaining neighbour of u
      v = g->adj[j];
      // if (g->lab[v]==l){
      g->lab[v] = l - 1;                  // label it
      g->sub[l - 1][g->ns[l - 1]++] = v;  // add it to the subgraph
      g->d[l - 1][v] = 0;                 // reset its degree
      //}
    }

    // reoder adjacency list and compute new degrees
    for (j = 0; j < g->ns[l - 1]; j++) {  // for each node in the subgraph
      v = g->sub[l - 1][j];
      end = g->cd[v] + g->d[l][v];
      for (k = g->cd[v]; k < end; k++) {  // for each remaining neighbour of v
        w = g->adj[k];
        if (g->lab[w] == l - 1) {         // if w is in the subgraph
          g->d[l - 1][v]++;               // increase degree of v
        } else {                          // otherwise
          g->adj[k--] = g->adj[--end];    // put w at the end of adj v
          g->adj[end] = w;
        }
      }
    }

    kclique(l - 1, g, n);                 // recursion on the subgraph

    for (j = 0; j < g->ns[l - 1]; j++) {  // restore modified labels
      v = g->sub[l - 1][j];
      g->lab[v] = l;
    }
  }
...
int main(int argc, char **argv) {
  if (argc < 3) {
    printf("Format not matched: ./kClist k edgelist.txt [order.txt]");
    exit(0);
  }
  specialsparse *g;
